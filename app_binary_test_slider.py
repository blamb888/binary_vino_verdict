...
# Define the conversion function with dynamic threshold
def convert_to_2_scale(arr, threshold):
    arr_2_scale = []
    for val in arr:
        if val < threshold:
            arr_2_scale.append(0)  # bad
        else:
            arr_2_scale.append(1)  # good
...
threshold = st.slider('What is your star rating threshold today?', 1, 5, 1, format="%d")
threshold_display = threshold - 1
...
if st.button('Predict'):
    ...
    predicted_classes_2_scale = convert_to_2_scale(predicted_classes, threshold_display)  # convert to 2 scale with dynamic threshold
